...
async fn sender(msg: String, ip:String) -> Result<(), Box<dyn Error>> {
 ...
 fn Elect_Server(sip:String,c_name:String,c:i32) -> Result<(), Box<dyn Error>>{
    println!("Electin!!");
 
     let cpu1:i32 = unsafe{cpu as i32};
     println!(" my cpu {}, ur cpu{}",cpu1,c);
    for item in clients.clone().lock().unwrap().iter_mut()
    {
        if(item.name.eq(&(c_name.clone())))
        {
            for server in item.servers.clone().lock().unwrap().iter_mut()
            {
                ...
                {
                    if(server.priority>unsafe{server_prio})
                    {
...
// async fn Process_Request(mm:String) -> Result<(), Box<dyn Error>> {
//     println!("Processing");
//     println!("mm {}",mm);
