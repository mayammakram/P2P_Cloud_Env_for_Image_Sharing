    ...
    else if req_type.eq("ip_request") || req_type.eq("ack-encrypt") || req_type.eq("offline_device")
    {
        ...
        for item in clients.clone().lock().unwrap().iter_mut() 
        {
            if item.name.eq(&(sep[0].clone())) 
            {
                ...
                let mut elected_server = "10.7.57.77".to_string();
                ...
                if elected_server.eq("10.7.57.77") 
                {
                    ...
                    { let echo_back_reply = sender(
                        "ack-encrypt 10.7.57.77:5001 awoo helo".to_string(),
...
async fn fault_tolerance() -> Result<(), Box<dyn Error>> { 
    while(true){
        if( unsafe { server_status == true}){
            ...
            sender("10.7.57.77 999".to_string(), "10.7.57.93:5053".to_string()).await;
            sender("10.7.57.77 999".to_string(), "10.7.57.31:5053".to_string()).await;
        ...
        rt.block_on(async {
                loop {
                    if(unsafe { server_status == true}){
                        ...
                        let _sender = sender(["10.7.57.77 ", unsafe{&percent_mem_usage.to_string()}].concat(),
                            "10.7.57.93:5053".to_string()).await;
                    
                        _sender;
                        let _sender_two = sender(["10.7.57.77 ", unsafe{&percent_mem_usage.to_string()}].concat(),
                        "10.7.57.31:5053".to_string()).await;
                        _sender_two;
