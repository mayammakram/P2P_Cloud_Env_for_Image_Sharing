...
// static mut client_ip: String = Mutex::new("0.0.0.0".to_string());
...
async fn listener() -> Result<String, Box<dyn Error>> {
 ...
 async fn listener_to_client_ack() -> Result<String, Box<dyn Error>> {
    ...
    // println!("listener to CLIENT TWO E");
 ...
 async fn update_rights(index: String, rights: String, client_ip: String, client_name: String) -> Result<(), Box<dyn Error>>
 {
    ...
    //     if i == index.parse::<usize>().unwrap()
    //     {
    //         let mut image_details_map = IMAGE_DETAILS_MAP.lock().unwrap();
    //         let _image_details = image_details {
    //             owner_name: value.owner_name.clone(),
    //             number_of_views: rights.parse::<u32>().unwrap(),
    //         };
    //         image_details_map.insert(key.to_string().clone(), _image_details);
    //     }
    // }
 ...
 async fn ask_for_input() -> String
 {
    ...
    //println!("To start, please enter your friend's computer name: ");
    let mut input = String::new();
        ...
        // println!("3. Update access rights of an image for a friend");
        std::io::stdin().read_line(&mut input).unwrap();

        // let sender_task = sender("10.7.57.31:5001".to_string(), ["maya".to_string()," ip_request ".to_string(), input.to_string(), " ayykalam ".to_string(), "0 ".to_string(), "false ".to_string()].concat()).await;
        // let sender_task = sender("10.7.57.77:5001".to_string(), ["maya".to_string()," ip_request ".to_string(), input.to_string(), " ayykalam ".to_string(), "0 ".to_string(), "false ".to_string()].concat()).await;
        // let sender_task = sender("10.7.57.93:5001".to_string(), ["maya".to_string()," ip_request ".to_string(), input.to_string(), " ayykalam ".to_string(), "0 ".to_string(), "false ".to_string()].concat()).await;
  
        //   println!("Your friend's computer name is: {}", input);
        input
 }
        ...
        {        // SEND IP_REQUEST
...

            {
                ...
                *friend_name_guard = "".to_string();
                // *friend_name_guard = server_ip.clone() + ":8080";
                // println!("AWOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO\nClient Name: {}", friend_name_guard);
                ...
                // println!("Please enter the image name: ");
                // std::io::stdin().read_line(&mut img_name).unwrap();
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // let mut client_ip = Mutex::new("0.0.0.0".to_string());


    //Client Reciever IP
    
    // let mut client_ip = "0.0.0.0".to_string();
                ...
                Ok(encoded_data) => 
                {
                    ...
                    if client_response_type.eq("view_image")
                    {
                        ...
                        if input_index.eq("last")
                        {
                            ...
                            // *client_ip = (server_ip.clone() + ":8080");
                        }
                    }
                    ...
                    if response_type.eq("encrypt")
                    {
                        ...
                        if last_packet_flag.eq("true")
                        {
                            ...
                            // if !image_details_map.contains_key("/home/aliali/Image_sharing_DS/client/src/decoded_msg_0.png") {
                            //     image_details_map.insert("/home/aliali/Image_sharing_DS/client/src/decoded_msg_0.png".to_string(), _image_details);
                            // }
                            }
                            println!("Image details map: {:?}", *IMAGE_DETAILS_MAP.lock().unwrap());
                    ...
                    match listener().await
                    {
                        Ok(encoded_data) => 
                        {
                            ...
                            if response_type.eq("ack-encrypt")
                            {
                                ...
                                // println!("Client IP in Serverr BEFORE{:?}", client_ip.clone());
                                        ...
                                        if let Some(hidden_image) = map.get(&server_ip.clone()) 
                                        {
                                            ...
                                            match text_to_image(&encoded_image_string) {
                                                Ok(encoded_img) => {
                                                    ...
                                                    // let image_path = "/home/aliali/Documents/Image_Sharing_DS/client/src/encoded_msg.png".to_string();
                                                    ...
                                                    // {
                                                        ...
                                                        // let sender_task = sender("10.7.57.77:5001" .to_string(), ["maya".to_string(),counter.to_string()," ack-encrypt ".to_string(), "ayykalam ".to_stinrg(), "0 ".to_string(), "false ".to_string() , "ayykalambardo".to_string()].concat()).await;
                                                        // counter += 1;
                                                    // }
                                                },
                                                Err(err) => {
                                ...
                                else
                                {
                                    ...
                                    let mut client_ip_guard = CLIENT_IP.lock().unwrap();
                                        ...
                                        *client_ip_guard = server_ip.clone() + ":8080";
                                        ...
                                        // let sender_task = sender(client_ip.clone(), ["maya ".to_string(), "10.7.57.87 ".to_string(), "view ".to_string(), "first ".to_string(), "0 ".to_string(), "false ".to_string() , base64_string_sample1.to_string()].concat()).await;
                                        // let sender_task = sender(client_ip.clone(), ["maya ".to_string(), "10.7.57.87 ".to_string(), "view ".to_string(), "last ".to_string(), "0 ".to_string(), "false ".to_string() , base64_string_sample1.to_string()].concat()).await;
